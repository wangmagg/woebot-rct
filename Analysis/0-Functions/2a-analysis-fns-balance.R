...
get_eot_taa_4_balance <- function(dat_input, grouping_var, get_smd) {
  dat_input <- dat_input |>
    mutate(eot_taa_4 = factor(eot_taa_4))
  bal_df <- dat_input |> .get_factor_var_bal_df('eot_taa_4', grouping_var)
  
  if (get_smd) {
    smd_df <- dat_input |> .get_smd_df('eot_taa_4', grouping_var)
    return (list('bal' = bal_df,
                 'smd' = smd_df))
  }
  return (list('bal' = bal_df))
...
balance_var_switch <- function(var_name, dat_input, grouping_var, get_smd) {
  switch(
    var_name,
...
get_balance <- function(dat_input, grouping_var, var_names, save_prefix, save_dir, get_smd=TRUE) {
  ...
  for (var_name in var_names) {
    res <- balance_var_switch(var_name, dat_input, grouping_var, get_smd)
  ...
  if (get_smd) {
    write.csv(all_smd, file.path(save_dir, str_c(save_prefix, 'smd.csv', sep='_')))
  }
}

#' Wrapper function for getting _only_ SMD information across specified variables
#' 
#' @param dat_input Data
#' @param grouping_var Name of variable that defines groups
#' @param var_names List of variables names to get balance table info for
#' @param save_prefix String prefix for name of file where results are saved
#' @param save_dir Directory to save results to
get_smd <- function(dat_input, grouping_var, var_names, save_prefix, save_dir) {
  if (!file.exists(save_dir)) {
    dir.create(save_dir)
  }
  all_smd <- data.frame()
  for (var_name in var_names) {
    res <- balance_var_switch(var_name, dat_input, grouping_var, get_smd=TRUE)
    all_smd <- bind_rows(all_smd, res$smd)
  }
  write.csv(all_smd, file.path(save_dir, str_c(save_prefix, 'smd.csv', sep='_')))
