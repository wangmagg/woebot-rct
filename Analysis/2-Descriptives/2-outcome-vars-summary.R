...
for (analysis_type in analysis_types) {
  ...
  save_dir <- file.path(ANALYSIS_OUT_DIR, '2-Descriptives-Output', '2-Outcome-Vars-Summary', analysis_type)
...
# Get balance descriptives for EOT TAA Goal item 
dat <- read_csv(file.path(DATA_OUT_DIR, 'dat_analysis_descriptive.csv'), show_col_types=FALSE)
dat_retained <- dat |> filter(retained_eot == 1)
save_dir <- file.path(ANALYSIS_OUT_DIR, '2-Descriptives-Output', '2-Outcome-Vars-Summary', 'itt', 'eot')
eot_taa_4_grped <- get_eot_taa_4_balance(dat_retained, c('group'), get_smd=FALSE)
eot_taa_4 <- get_eot_taa_4_balance(dat_retained, c(), get_smd=FALSE)
write.csv(eot_taa_4_grped$bal, file.path(save_dir, str_c("grouped", "eot_taa_4", 'balance.csv', sep='_')), row.names=FALSE)
write.csv(eot_taa_4$bal, file.path(save_dir, str_c("eot_taa_4", 'balance.csv', sep='_')), row.names=FALSE)