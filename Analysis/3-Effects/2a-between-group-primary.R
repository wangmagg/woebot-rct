...
for (analysis_type in analysis_types) {
  ...
  save_dir <- file.path(ANALYSIS_OUT_DIR, '3-Effects-Output', '2a-Between-Group-Primary', analysis_type)
  ...
  for (timept in timepts) {
    ...
    for (analysis in analyses_to_run) {
      ...
      if (analysis == 'ttest') {
        ...
        # calculate cohen's d
        if (!dir.exists(file.path(save_dir, timept, 'cohen'))) {
          dir.create(file.path(save_dir, timept, 'cohen'), recursive = TRUE)
        }
        get_cohens_d(dat_to_fit,
                     outcome_var = outcome_var,
                     retain_var = retain_var,
                     save_dir = file.path(save_dir, timept, 'cohen'))
